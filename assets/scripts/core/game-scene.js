          ...
          if (PROXY_BASE && songId > 0) {
              try {                  
                  ...
                  if (ngText && ngText !== "-1") {
                      ...
                      let songUrl = decodeURIComponent((ngMap["10"] || "").trim());
                      const songArtist = (ngMap["4"] || "Unknown").replace(/:$/, "").trim();
                      const songTitle = (ngMap["2"] || `Song #${songId}`).replace(/:$/, "").trim();
                      const nongType = parseInt(ngMap["11"] || "0");
                      const isNCS = songId >= 1000000 || nongType === 1;
                      ...
                      if (!songUrl && isNCS) {
                          ...
                          songUrl = `https://geometrydashfiles.b-cdn.net${path}?token=${token}&expires=${expires}`;
                      }
